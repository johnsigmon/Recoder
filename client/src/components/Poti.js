...
class Poti extends React.Component {
  ...
  componentWillMount() {
    ...
    this.updateValue(this.initialDeg, true);
  }
  componentWillReceiveProps(newProps) {
    // make sure inital options are applied
      let deg = ((newProps.value || 0) - this.range[0]);
      deg /= (this.range[1] - this.range[0]);
      deg *= this.fullAngle;
      deg += this.startAngle;
    this.updateValue(deg, true);
  }
  ...
  updateValue(deg, silent) {
    ...
    if (typeof this.props.onChange === 'function' && !silent) {
  ...
  render() {
    ...
    Object.assign(styles.marker, dynamicStyles.marker);
